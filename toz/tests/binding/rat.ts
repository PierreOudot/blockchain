...
const getBalance_arg_to_mich = (): att.Micheline => {
    return att.unit_mich;
}
export class Rat_transfer {
    ...
    async getBalance(params: Partial<ex.Parameters>): Promise<att.CallResult> {
        if (this.address != undefined) {
            return await ex.call(this.address, "getBalance", getBalance_arg_to_mich(), params);
        }
        throw new Error("Contract not initialised");
    }
    ...
    async get_getBalance_param(params: Partial<ex.Parameters>): Promise<att.CallParameter> {
        if (this.address != undefined) {
            return await ex.get_call_param(this.address, "getBalance", getBalance_arg_to_mich(), params);
        }
        throw new Error("Contract not initialised");
    }
    errors = {
