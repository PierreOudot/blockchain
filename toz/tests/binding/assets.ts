...
const add_vehicle_arg_to_mich = (pvin: string, ncolor: Color): att.Micheline => {
    return att.pair_to_mich([
        att.string_to_mich(pvin),
        ncolor.to_mich()
    ]);
}
...
export class Assets {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./contracts/assets.arl", {}, params)).address;
        this.address = address;
    }
    async add_vehicle(pvin: string, ncolor: Color, params: Partial<ex.Parameters>): Promise<att.CallResult> {
        if (this.address != undefined) {
            return await ex.call(this.address, "add_vehicle", add_vehicle_arg_to_mich(pvin, ncolor), params);
        }
        throw new Error("Contract not initialised");
    }
    ...
    async get_add_vehicle_param(pvin: string, ncolor: Color, params: Partial<ex.Parameters>): Promise<att.CallParameter> {
        if (this.address != undefined) {
            return await ex.get_call_param(this.address, "add_vehicle", add_vehicle_arg_to_mich(pvin, ncolor), params);
        }
        throw new Error("Contract not initialised");
    }
